...
class PaymentEventPublisherTest {
   ...
   void setUpRawConsumer() {
      ...
      Map<String, Object> props = KafkaTestUtils.consumerProps( "pub-verify-group", "true", embeddedKafkaBroker );
   ...
   void initiatePayment_publishesFailedEvent_whenGatewayDeclines() {
      stubRepositoryAndGateway( false );
   ...
   void idempotentProducer_doesNotDuplicateMessage_onRetry() throws Exception {
      ...
      var records = KafkaTestUtils.getRecords( rawConsumer, 3000L );
      assertThat( records.count() ).isEqualTo( 2 ); // both arrive — consumer must be idempotent
   }

   // ── Helpers ───────────────────────────────────────────────────────────────

   private void stubRepositoryAndGateway( boolean gatewaySuccess ) {
      Payment pending = buildPayment( 1L, PaymentStatus.PENDING );
      Payment terminal = buildPayment( 1L, gatewaySuccess ? PaymentStatus.CAPTURED : PaymentStatus.FAILED );
